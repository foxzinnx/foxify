...
export const PostItem = ({ post }: Props) => {
    ...
    return(
        <div className="">
            <div className="bg-white border border-neutral-200 drop-shadow-xs">
                {showRepostDiv && (
                    <div className={`flex gap-2 items-center px-5 pt-3 transition-all duration-300 ease-in-out ${isAnimating ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform -translate-y-2'}`}>
                        <FontAwesomeIcon icon={faRepeat} className="size-[17px] text-gray-600" />
                        <p className="text-gray-600 font-semibold text-[13px]">{user.slug} repostou</p>
                    </div>
                )}
